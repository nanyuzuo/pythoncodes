...
with open("d:\\yequ\\test\\美食排行.xlsx", "rb") as food_file:
...
with open("d:\\yequ\\test\\城市景点.xlsx", "rb") as view_file:
...
mailUser = "nanyuzuo@163.com"
# 邮箱授权码设置，"wiscbwlrcodprabd"赋值给mailPass 
mailPass = "OUPEADXPJDIYBKGE"
...
# 定义一个字典mail_dict={}
mail_dict = {"nanyu":"115185849@qq.com", "北落":"nanyualpha@hotmail.com"}
...
for key, value in mail_list:
    ...
    message['From'] = Header(f"旅游达人胡为<{mailUser}>")
    # 使用格式化设置邮件收件人名称 XX同学<xxxx@qq.com>
    message['To'] = Header(f"{key}同学<{value}>")
    # 设置邮件主题 北京和上海旅游信息汇总
    message['Subject'] = Header(f"{today}北京和上海旅游信息汇总")

    # TODO 使用message.attach()函数上传邮件正文

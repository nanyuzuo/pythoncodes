...
for i in range(0,10):
    ...
    for idx,content in enumerate(content_all,1):
        ...
        # 用格式化将图片名字和.jpg格式组合
        # 打开的文件赋值为f
        with open(f"d:\\yequ\\movie\\{imgName}.jpg","wb") as f:
            # TODO 使用write()将图片写入
            f.write(img)
    print(f"第{start}名至第{end}名的海报已成功爬取！")
print("全部海报爬取完毕！")
...
# TODO 添加data_only = True，读取工作目录里名为"平台销售订单.xlsx"的工作簿并赋值给变量wb_mall
...
for row in ws_mall.rows:

    # TODO 读取每一行的第一个单元格值，赋值给变量mall_order_id
    mall_order_id = row[0].value
...
# TODO 添加data_only = True，读取工作目录里名为"系统数据.xlsx"的工作簿并赋值给变量wb_system
...
for row in ws_system.rows:
    # TODO 读取每一行的第二个单元格值，赋值给变量system_order_id
    system_order_id = row[1].value
